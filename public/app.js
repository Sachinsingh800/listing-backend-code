...
elements.form.addEventListener("submit", async (event) => { event.preventDefault(); if (!elements.file.files[0]) return; elements.importButton.disabled = true; elements.importButton.textContent = "Importing…"; elements.result.innerHTML = ""; try { const response = await fetch("/api/products/import", { method: "POST", body: new FormData(elements.form) }); const data = await response.json(); const errors = (data.errors || []).slice(0, 5).map((error) => `<li>${escapeHtml(`${error.sheet}, row ${error.row}: ${error.message}`)}</li>`).join(""); elements.result.innerHTML = `<div class="${response.ok || response.status === 207 ? "result-success" : "result-error"}"><strong>${escapeHtml(data.message || "Import finished.")}</strong> ${data.imported ?? 0} created, ${data.updated ?? 0} updated, ${data.failed ?? 0} failed.${errors ? `<ul class="result-errors">${errors}</ul>` : ""}</div>`; if (response.ok || response.status === 207) { elements.form.reset(); elements.fileName.textContent = "Choose Excel or CSV file"; await loadProducts(); } } catch (error) { elements.result.innerHTML = `<p class="result-error">${escapeHtml(error.message || "Import failed.")}</p>`; } finally { elements.importButton.disabled = false; elements.importButton.textContent = "Import file"; } });
loadProducts();
